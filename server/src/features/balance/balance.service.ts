...
import { EmployeeRepository } from '../employee/employee.db.js';
...
export class BalanceService {
   ...
   * formula in it; ../features/entitlement/annual-grant.job.ts passes such a person over and reports that it
   * did.
   */
   ...
   *
   * The deadline arrives already resolved — `expiryFor` in ../features/leave-event/leave-event.ts,
   ...
   * decided by `decideTheLapse` in ../features/leave-event/leave-event.ts, which is also what refuses
   ...
   * balance, decided by `decideTheCarry` in ../features/leave-year/year-rollover.ts and arriving here
   ...
   * closed.** ../features/leave-year/year-rollover.job.ts closes the year before it reads a single figure out
   ...
   * over.
   *
   * Nothing is subtracted from the year that closed; see ../features/leave-year/year-rollover.ts for
   ...
   *
   * **An HR Administrator's, and nobody else's** — see ../features/balance/policy.ts. The
   ...
   * ../features/balance/ledger.ts argues is no use to a screen. See
   ...
   *
   *   **The rule is decided inside the window**, by ../features/balance/balance.ts, which is
   ...
   * ../features/balance/ledger.ts says a screen cannot use. With it they are told which of the
